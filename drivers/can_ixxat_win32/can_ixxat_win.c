...
{	
	...
	*/
	...
	//JDA if only one device found use it instead of open selection dialog
	HANDLE hEnum;
	VCIDEVICEINFO pInfo;
	char foundOne = 0;
	if (vciEnumDeviceOpen(&hEnum) == VCI_OK)
	{
		if (vciEnumDeviceNext(hEnum, &pInfo) == VCI_OK)
		{
			VCIDEVICEINFO other;
			if (vciEnumDeviceNext(hEnum, &other) != VCI_OK)
				foundOne = 1;
		}
	...
	{
		hResult = vciDeviceOpen(&pInfo.VciObjectId, &hDevice);
	}
	// end JDA
